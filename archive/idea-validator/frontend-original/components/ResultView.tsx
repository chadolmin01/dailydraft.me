...
import { generateFinalArtifacts } from '../services/geminiService';
...
const ResultView: React.FC<ResultViewProps> = ({ conversationHistory, originalIdea, reflectedAdvice = [] }) => {
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto py-8 px-4 md:px-8">
      ...
      <div className="mb-8 bg-white border border-gray-200 rounded-xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between shadow-sm">
          ...
          <div className="mt-6 md:mt-0 text-right hidden md:block border-l border-gray-100 pl-8">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        ...
        <div className="xl:col-span-7 lg:col-span-8">
           <div className="relative">
               ...
               {/* Mobile Tabs */}
               <div className="xl:hidden flex gap-2 mb-8 p-1 bg-gray-100 rounded-lg overflow-x-auto">
                   <button onClick={() => setActiveTab('overview')} className={`flex-1 py-2 px-4 whitespace-nowrap text-sm font-bold rounded-md transition-all ${activeTab === 'overview' ? 'bg-white shadow-sm text-black' : 'text-gray-500'}`}>Overview</button>
                   <button onClick={() => setActiveTab('prd')} className={`flex-1 py-2 px-4 whitespace-nowrap text-sm font-bold rounded-md transition-all ${activeTab === 'prd' ? 'bg-white shadow-sm text-black' : 'text-gray-500'}`}>PRD v1.0</button>
                   <button onClick={() => setActiveTab('jd')} className={`flex-1 py-2 px-4 whitespace-nowrap text-sm font-bold rounded-md transition-all ${activeTab === 'jd' ? 'bg-white shadow-sm text-black' : 'text-gray-500'}`}>JD</button>
               </div>
