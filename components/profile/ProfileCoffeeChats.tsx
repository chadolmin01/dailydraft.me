                ...
                <div key={chat.id} className="bg-surface-card border border-border-strong p-4 hover:shadow-sharp transition-all">
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 border flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                      chat.status === 'accepted' ? 'bg-status-success-bg border-indicator-online/30 text-indicator-online' :
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className={`text-[0.625rem] font-mono font-bold px-1.5 py-0.5 border ${
                          ...
                          chat.status === 'accepted' ? 'bg-status-success-bg text-indicator-online border-indicator-online/20' :
                          'bg-surface-sunken text-txt-tertiary border-border'
                        }`}>
                          {chat.status === 'pending' ? 'PENDING' : chat.status === 'accepted' ? 'ACCEPTED' : 'DECLINED'}
                        </span>
                      </div>
                      {chat.message && (
