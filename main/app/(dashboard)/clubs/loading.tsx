      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-16">
        <div className="space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="h-9 w-28 skeleton-shimmer rounded-full" />
          </div>
          {/* 검색바 */}
          ...
          <div className="flex gap-2 overflow-hidden">
            {[0, 1, 2, 3, 4].map(i => (
              <div key={i} className="h-8 w-20 skeleton-shimmer rounded-full shrink-0" />
            ))}
          </div>
          {/* 카드 그리드 */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {[0, 1, 2, 3, 4, 5].map(i => (
              <div key={i} className="flex gap-3.5 p-4 bg-surface-card border border-border rounded-xl">
                <div className="w-[52px] h-[52px] skeleton-shimmer rounded-md shrink-0" />
