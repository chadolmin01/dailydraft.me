/**
 * GitHub OAuth2 — 클럽에 GitHub 계정 연결
 *
 * 클럽 설정에서 "GitHub 연결" 클릭 시 이 엔드포인트로 이동.
 * GitHub OAuth 인증 페이지로 리다이렉트한다.
 *
 * 플로우:
 * 1. GET /api/github/oauth?clubId=xxx → GitHub 인증 페이지로 리다이렉트
 * 2. GitHub에서 승인 → /api/github/oauth/callback으로 돌아옴
 * 3. callback에서 access_token 교환 → club_harness_connectors에 저장
 *
 * state 파라미터에 clubId를 포함하여 콜백에서 어느 클럽인지 식별.
...
export async function GET(req: NextRequest) {
  ...
  // state에 clubId + clubSlug + CSRF nonce를 인코딩
  // nonce는 콜백에서 검증하지는 않지만, replay attack 방어에 도움
  const nonce = crypto.randomBytes(16).toString('hex')
  const state = Buffer.from(
    JSON.stringify({ clubId, clubSlug, nonce })
  ).toString('base64url')
