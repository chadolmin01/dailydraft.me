    ...
    <div>
        ...
        <div className="flex flex-wrap gap-1 mb-2">
          {tags.map(tag => (
            <span key={tag} className="flex items-center gap-1 px-2 py-0.5 text-xs font-medium bg-brand text-white">
              {tag}
              <button type="button" onClick={() => remove(tag)} className="hover:opacity-70 p-2 sm:p-0 -m-1 sm:m-0" aria-label={`${tag} 제거`}><X size={10} /></button>
            </span>
          ))}
        </div>
      )}
      ...
      <div className="flex gap-1.5">
        <input
          ...
          className="flex-1 px-3 py-1.5 text-base sm:text-sm border border-border bg-surface-card rounded-lg ob-input"
        />
...
export const EditAIProfile: React.FC<EditAIProfileProps> = ({
  ...
  return (
    <section>
      ...
      <p className="text-xs text-txt-tertiary mb-4">온보딩 AI 대화에서 분석된 데이터입니다. 틀리거나 달라진 항목은 직접 수정하실 수 있습니다.</p>
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder="예: 15"
              className="w-32 px-3 py-2 text-base sm:text-sm border border-border bg-surface-card rounded-lg ob-input"
            />
