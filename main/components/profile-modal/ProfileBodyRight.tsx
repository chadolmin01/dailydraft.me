...
function CollapsibleSection({ children, title, icon: Icon, defaultOpen = true, count }: {
  ...
  return (
    <section>
      ...
      >
        <span className="w-8 h-8 bg-[#F2F3F5] dark:bg-[#2C2C2E] text-txt-secondary flex items-center justify-center rounded-xl">
          <Icon size={15} />
        </span>
        <span className="text-[15px] font-bold text-txt-primary">{title}</span>
        {count != null && (
          <span className="text-[12px] font-semibold text-[#3182F6]">{count}</span>
        )}
...
function TraitBadge({ label, value }: {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 bg-[#F7F8F9] dark:bg-[#1C1C1E] rounded-2xl">
...
export function ProfileBodyRight({
  ...
  return (
    <div className="md:col-span-2 space-y-4 md:bg-[#F7F8F9] md:dark:bg-[#1C1C1E] md:rounded-2xl md:p-5 md:self-start">
      {/* Empty state */}
      {isEmpty && (
        <div className="px-4 py-10 bg-[#F7F8F9] dark:bg-[#1C1C1E] text-center rounded-2xl">
      ...
      {hasSkills && (
        <section>
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <span className="text-[12px] font-semibold text-[#3182F6]">{skills!.length}</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {skills!.map((skill) => (
              <span
                key={skill.name}
                className="px-3 py-1.5 text-[13px] font-medium rounded-full bg-[#F2F3F5] dark:bg-[#2C2C2E] text-txt-primary hover:bg-[#E5E5EA] dark:hover:bg-[#3A3A3C] transition-colors"
              >
                {skill.name}
              </span>
            ))}
