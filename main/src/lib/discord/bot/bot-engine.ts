...
import { buildLauncherMenu } from './modals';
import { canPushToday } from './daily-limiter';
import { shouldAutoSuppress, invalidateAcceptanceCache } from './acceptance-tracker';
import {
  saveIntervention,
...
// @멘션 대화용 시스템 프롬프트
// 멘션 후 질문 텍스트가 있을 때만 AI Q&A로 진입 (질문이 비어 있으면 버튼 런처 메뉴로 전환)
...
- 프로젝트 진행 관련 조언
- 기능을 쓰려면 "@Draft" 만 멘션하면 버튼 메뉴가 뜬다고 안내
...
export class BotEngine {
  ...
  async onMessage(raw: {
    ...
    // 즉시 반응 후보 — pushEnabledPatterns 에 속한 것만 허용
    // 이유: 연구 권장선(채널당 하루 2~3회) 준수 및 디지털 피로 방지.
    //      요약 파이프라인으로 축적할 패턴은 여기서 Push 하지 않음.
    const instantCandidates = candidates.filter(
      (c) =>
        this.config.pushEnabledPatterns.includes(c.type) &&
        c.confidence >= 0.5 &&
    ...
    for (const candidate of instantCandidates) {
      // 일일 채널 개입 상한 체크
      const canPush = await canPushToday(
        msg.channelId,
        this.config.channelDailyPushLimit,
      );
      if (!canPush) {
        console.log(
          `[BotEngine] 채널 ${msg.channelId} 일일 Push 상한 도달 — ${candidate.type} 스킵`,
        );
        break;
      }

      // 수락률 자동 억제 체크
      const suppressed = await shouldAutoSuppress(
        msg.channelId,
        candidate.type,
        this.config.acceptanceRateThreshold,
        this.config.acceptanceRateMinSample,
      );
      if (suppressed) {
        console.log(
          `[BotEngine] ${candidate.type} 수락률 저조 — 자동 억제 (채널 ${msg.channelId})`,
        );
        continue;
      }
      ...
      if (response) {
        ...
        invalidateAcceptanceCache(msg.channelId, candidate.type);
    ...
    // 종결 신호 감지 → 마무리 요약 예약
    if (hasEndSignal) {
      // 비즉시 패턴도 분석 (ruleOnlyPatterns는 AI 건너뜀)
      const allDetections = await detectPatterns(
        messages,
        this.config.ruleOnlyPatterns,
      );
  ...
  private async handleSlashCommand(msg: BufferedMessage): Promise<void> {
    ...
    switch (cmd) {
      ...
      case '/help': {
        await this.sendResponse({
          content: `📖 **Draft 봇 사용법**\n\n**주요 진입: \`@Draft\` 멘션**\n봇을 멘션하면 버튼 메뉴가 뜹니다 — 투두/투표/한줄/일정/회의시작/마무리/개발현황/설정.\n\n**슬래시 커맨드 (파워유저용):**\n• \`/마무리\` — 지금까지 대화 요약 (AI)\n• \`/개발현황\` — 오늘의 개발 활동 로그\n• \`/github\` — GitHub 레포 연동 관리\n\n**대화:**\n• \`@Draft 질문내용\` — AI에게 질문\n\n**자동 감지:**\n• 일정 확정 → Discord Event 자동 등록\n• 대화 종결 → 회의 요약 자동 생성`,
          channelId: msg.channelId,
        });
        break;
      }
  ...
  /**
   * @멘션 대화 처리
   * - "@Draft" 만 치면 → 버튼 런처 메뉴 (60초 후 자동 삭제)
   * - "@Draft 오늘 할 일 뭐야?" → 최근 대화 컨텍스트 포함하여 AI 응답
   *
   * 이유 주석:
   * Discord 멘션은 interaction이 아니라 일반 메시지라 ephemeral(본인만 보이기) 불가.
   * 따라서 공개 메시지로 보내고 60초 뒤 자동 삭제해 노이즈를 억제합니다.
   * 삭제 실패해도(권한·타이밍) 치명적이지 않으므로 로그만 남기고 진행.
   */
  private async handleMention(msg: BufferedMessage): Promise<void> {
    ...
    // 멘션만 있고 질문이 비면 → 런처 메뉴 진입
    if (!query || /^[\s\p{P}]+$/u.test(query)) {
      const menu = buildLauncherMenu(msg.authorId);
      ...
      // 60초 후 자동 삭제 (권한 실패해도 메시지는 남아 있음 — 치명적 X)
      if (sent?.id) {
        setTimeout(() => {
          deleteChannelMessage(msg.channelId, sent.id).catch((err) =>
            console.warn('[BotEngine] 런처 메뉴 자동 삭제 실패:', err),
          );
        }, 60_000);
      }
      return;
    ...
    try {
      ...
      if (reply) {
        ...
        // /연결 슬래시 커맨드는 archived 되었으므로 웹 프로필 연결로 안내
        if (!this.nicknameCache.has(msg.authorId)) {
          reply += '\n\n-# 💡 Draft 웹에서 Discord 계정을 연결하면 더 정확한 답변을 받을 수 있습니다';
        }
        await this.sendResponse({
          content: reply,
          channelId: msg.channelId,
  ...
  private async triggerSummary(channelId: string): Promise<void> {
    ...
    if (pending.length === 0) {
      ...
      const detections = await detectPatterns(messages, this.config.ruleOnlyPatterns);
